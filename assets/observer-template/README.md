...
这是一个仅在本机运行的只读观测台。它汇总共享工作区中的 Codex、DSH、WorkBuddy、协调状态及结构化工作流事件，提供 JSON 快照、SSE 事件流与实时调用 DAG。缺少证据时显示 `unknown` 或“暂无证据”，不会生成完成百分比和预计时间。

调用图采用用户选定的 A·DAG 四层总览：当前任务为根，分出 Codex、DSH、WorkBuddy，再经三张调用链摘要汇入观测聚合。九条总览边表达归属/汇聚关系，不冒充原始事件父链；真实 `events[].parent_id → events[].id` 在右侧事件明细中保留。每条总览边都有独立稳定的颜色、持续可见底轨、流动高光与落在卡片边框上的端子。工具栏可缩放、适配画布及暂停流动；右侧可筛选来源/状态，并分区展示每个 Agent 的实时进展、交付文件、测试、产物、风险与 handoff，以及 Task ID、Agent/session ID、模型、effort、状态依据、时间、工具、耗时、退出码和来源健康。窗口或面板变窄时会重算三分支布局而不是把整图压成不可读的缩略图，实时刷新后也会保留当前 DAG 节点的键盘焦点。
...
正常多 Agent 工作流应由 Skill 的 `scripts/start-workflow.ps1` 启动；它会刷新模板、复用或隐藏启动服务、完成健康检查，并默认打开浏览器。
